<script type="text/html" data-template-name="blockChain function">
  <style>
      .func-tabs-row {
          margin-bottom: 0;
      }
  ...
  <div id="func-tabs-content" style="min-height: calc(100% - 95px);">
      ...
      <div id="func-tab-init" style="display:none">
          <div class="form-row node-text-editor-row" style="position:relative">
              <div style="height: 250px; min-height:150px;" class="node-text-editor" id="node-input-init-editor" >
              </div>
      ...
      <div id="func-tab-body" style="display:none">
          <div class="form-row node-text-editor-row" style="position:relative">
              <div style="height: 220px; min-height:150px;" class="node-text-editor" id="node-input-func-editor" >
                <!-- 以内嵌iframe的方式把chainIDE给放入 -->
                <!-- TODO: 合约需要可以传入参数，可能要对应去ChainIDE里面看具体的方法 -->
                <!-- <iframe src="https://red-chainide.oss-cn-beijing.aliyuncs.com/s/ethereum/49963693c01b4ffba9b39cf5cc1a035c?open=Storage.sol&type=file" title="ChainIDE" id="ChainIDE" frameBorder="0" scrolling="no"></iframe> -->
                <!-- TODO: 可以手动上传两个模板，然后看能否自动部署 -->
                <!-- TODO: 现在问题转移到：如何接收配置参数？ -->
                <!-- 这个是low-code-chainide -->
                <!-- https://red-chainide.oss-cn-beijing.aliyuncs.com/s/ethereum/3b9e1a95918c400198e91b2e487034aa?open=Store.sol&type=file -->
              </div>
      ...
      <div id="func-tab-finalize" style="display:none">
          <div class="form-row node-text-editor-row" style="position:relative">
              <div style="height: 250px; min-height:150px;" class="node-text-editor" id="node-input-finalize-editor" >
              </div>
...
(function () {
  ...
  RED.events.on("nodes:add", function (n) {
    if (n.type === 'inject') {
      for (var i = 0; i < n.props.length; i++) {
        if (n.props[i].p === 'contractType') {
          switch(n.props[i].v) {
            case 'read':
              msg = 'https://red-chainide.oss-cn-beijing.aliyuncs.com/s/ethereum/8e068a241d1c43ab9e3a0d586d6b94e2?open=Read.sol&type=file';
              return;
            case 'storage':
              msg = 'https://red-chainide.oss-cn-beijing.aliyuncs.com/s/ethereum/3b9e1a95918c400198e91b2e487034aa?open=Store.sol&type=file';
              return;
            default:
              msg = 'https://red-chainide.oss-cn-beijing.aliyuncs.com/s/ethereum/49963693c01b4ffba9b39cf5cc1a035c?open=Storage.sol&type=file';
          }
        }
      }
    }
    if (n.type === "blockChain function") {
  ...
  var ideContanierInitWidth = 0;
  var ideContanierExpanded = false;
  ...
  RED.events.on("runtime-state", function (event) {
    if (event.error === "missing-modules") {
      ...
      for (var id in knownFunctionNodes) {
        ...
        if (knownFunctionNodes.hasOwnProperty(id) && knownFunctionNodes[id].props) {
          msg += JSON.stringify(knownFunctionNodes[id].props);
        }
      }
    }
    RED.view.redraw();
  ...
  // 注册blockChain function节点
  RED.nodes.registerType('blockChain function', {
    color: "#C0C0C0",
    category: 'function',
    defaults: {
      name: { value: "blockChain function" },
      func: { value: "\nreturn msg;" },
    ...
    oneditprepare: function () {
      ...
      this.editor = buildEditor('node-input-init-editor', $("#node-input-func").val(), undefined, that.libs || [])
      this.finalizeEditor = buildEditor('node-input-finalize-editor', $("#node-input-finalize").val(), RED._("node-red:function.text.finalize"))
      msg && $("#node-input-func-editor").append('<iframe src="' + msg + '" title="ChainIDE" id="ChainIDE" frameBorder="0" scrolling="no"/>')
      !msg && $("#node-input-func-editor").append('<div>Please add config in inject first!</div>')
      ...
      ideContanierInitWidth = $("#red-ui-editor-stack>div.red-ui-tray").css("width");
      var expanIdeButtonClickHandler = function (e) {
        e.preventDefault();

        var contanier = $("#red-ui-editor-stack>div.red-ui-tray");
        if(ideContanierExpanded) {
          contanier.css("width", ideContanierInitWidth)
          ideContanierExpanded = false
        } else {
          contanier.css("width", "calc(100vw - 333px)")
          ideContanierExpanded = true
        }

      }

      $("#node-init-expand-js").on("click", expandButtonClickHandler(this.initEditor));
      $("#node-function-expand-js").on("click", expanIdeButtonClickHandler);
