...
module.exports = function (RED) {
    ...
    var { AbiHelper } = require('@white-matrix/web3-solidity-wallet');
    var { Provider, extend } = require('@remix-project/remix-simulator');

    var web3 = new Web3();
    var abiHelper = new AbiHelper(web3);
        ...
        const res =  new Promise(function(resolve, reject) {
            try {
                provider
                    .init()
                    .then(function() {
                        return provider.Accounts.resetAccounts;
                    })
                    .then(function() {
                        web3.setProvider(provider);
                        extend(web3);
                        web3.eth.handleRevert = true;
                        abiHelper = new AbiHelper(web3);
    ...
    async function deployContract(msg) {
        ...
        return new Promise((resolve, reject) => {
            if (!accountList.length) {
                reject(new Error('no account created'));
            }
            console.log('in deploy contract');
            const { abi } = msg.compiledOutput;
                const constructorFunc = abi.find(function (o) {
                    return o.type === 'constructor';
                });
                try {
                    console.log('in deploy try')
                    var params = abiHelper?.encodeParameters(
                        constructorFunc?.inputs || [],
                        // TODO: 这个是啥
                        // msg.constructorArgs || ''
                        undefined
                    );
                    const bytecode = `0x${msg.compiledOutput.evm.bytecode.object}`;
                    ...
                    web3?.eth
                        .sendTransaction({
                            ...
                            // value: msg.deployOptions.payPrice,
                        ...
                        //         });
                        //         resolve({ receipt, confirmationNumber: confNumber })
                        //     }
                        // )
            ...
            //     // TODO: 这个应该从contract里获取，有一个api的数组
            //     /**
            //     * output的结构如下
            //     * abi数组: inputs[],name,outputs[],stateMutability,type
            //     * devdoc: details, kind, methods(编译之后的sol方法),title,version
            //     * evm: assembly, bytecode(functionDebugData, generateSources, linkReferences, object, opcodes, sourceMap),deployBytecode(functionDebugData, generatedSources, immutableReferences, linkReferences, object, opcodes, sourceMap), gasEstimates(creation, external), legacyAssembly(.code,.data,sourceList), methodIdentifiers
            //     * ewasm: wasm
            //     * metadata
            //     * storageLayout
            //     * userdoc
