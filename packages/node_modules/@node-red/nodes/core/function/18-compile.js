...
module.exports = function (RED) {
    ...
    async function compileContract(msg) {

        var contentSource;

        await axios.get('https://gateway-beta.chainide.com/fs/getFileContent', {
            params: {
                workspaceId: '49963693c01b4ffba9b39cf5cc1a035c',
                filePath: '49963693c01b4ffba9b39cf5cc1a035c/Storage.sol'
            }
        }).then(function(res) {
            // console.log('success', JSON.parse(res.data.data).content);
            contentSource = JSON.parse(res.data.data).content;
        }).catch(function(err) {
        ...
        const response = new Promise(function (resolve, reject) {
            try {
                ...
                // TODO: 可以强行直接用最新版本去编译，注意上传的合约里面暂时都要设置一下最高版本
                resolve(JSON.parse(solc.compile(JSON.stringify(input))));
            } catch (e) {
                /* handle error */
    ...
    function CompileNode(n) {
        ...
        node.on("input", function (msg, send, done) {
            if (state === RESOLVING) {
                ...
                compileContract(msg)
                    .then(function(res) {
                        msg.compileResult = 'compile success, waiting for deploy';
                        msg.compiledOutput = res.contracts[`${msg.contractType || 'Storage'}.sol`].Storage;
                        // console.log('compile success', 'output', res.contracts[`${msg.contractType || 'Storage'}.sol`].Storage.evm);
                        // , 'source', res.sources,
                    })
                    .catch(function(err) {
                        msg.compileResult = 'compile error, please try again';
    ...
    RED.nodes.registerType("compile", CompileNode, {
